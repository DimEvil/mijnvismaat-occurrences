...
```{r}
input_interim %<>% mutate(decimalLongitude = `Lat (EPSG: 4326)`) %>%
                   mutate(decimalLatitude = `Lon (EPSG: 4326)`)
```
...
```{r}
occurrence %<>% mutate(rightsHolder = "MijnVISmaat, Sportvisserij Nederland") # e.g. "INBO"
```
### accessRights
...
```{r}
occurrence %<>% mutate(institutionCode = "Sportvisserij Nederland") # e.g. "INBO"
```

### datasetName

```{r}
occurrence %<>% mutate(datasetName = "MijnVISmaat application occurrences in Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
occurrence %<>% mutate(samplingProtocol = 'catch') 
...
```{r}
occurrence %<>% mutate(locality = Visstek)
```
...
###identifiedBy
```{r}
occurrence %<>% mutate(identifiedBy = 'MijnVISmaat, Sportvisserij Nederland')
```
...
col_order <- c( "ID","type","language","license","rightsHolder","accessRights","datasetID"
               ...
               ,"individualCount","occurrenceStatus","samplingProtocol","eventDate","continent","countryCode","locality"
